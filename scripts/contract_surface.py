...
RISK_BY_NAME: dict[str, RiskTier] = {
    ...
    "isVerified": "low",
    "investorCountry": "low",
    "investorIdentity": "low",
    "setComplianceRules": "medium",
    "canTransfer": "low",
    "setDiligenceAttestationRegistry": "medium",
    "setRecoveryDelay": "medium",
    ...
    "burn": "high",
    "forcedTransfer": "high",
    "proposeRecoveryAddress": "medium",
    "cancelRecoveryAddress": "medium",
    "executeRecoveryAddress": "high",
    "getRecoveryRequest": "low",
    ...
    "removeMinter": "medium",
    "isMinter": "low",
    "addRecoveryOperator": "medium",
    "removeRecoveryOperator": "medium",
    "isRecoveryOperator": "low",
    "pause": "medium",
    "unpause": "medium",
    "holderCount": "low",
    "maxHolders": "low",
...
CAST_HINTS: dict[str, str] = {
    "registerIdentity": "cast send <token> \"registerIdentity(address,uint16,bytes32)\" <investor> <country> <identityId> --rpc-url $RPC --private-key $PK",
    "batchRegisterIdentity": "cast send <token> \"batchRegisterIdentity(address[],uint16[],bytes32[])\" \"[<a1>]\" \"[<c1>]\" \"[<id1>]\" --rpc-url $RPC --private-key $PK",
    ...
    "mint": "cast send <token> \"mint(address,uint256,bytes32)\" <to> <amount> <evidenceHash> --rpc-url $RPC --private-key $PK",
    ...
    "proposeRecoveryAddress": "cast send <token> \"proposeRecoveryAddress(address,address,bytes32)\" <lost> <new> <identityId> --rpc-url $RPC --private-key $PK",
    "cancelRecoveryAddress": "cast send <token> \"cancelRecoveryAddress(bytes32)\" <requestId> --rpc-url $RPC --private-key $PK",
    "executeRecoveryAddress": "cast send <token> \"executeRecoveryAddress(bytes32)\" <requestId> --rpc-url $RPC --private-key $PK",
    ...
    "addMinter": "cast send <token> \"addMinter(address)\" <account> --rpc-url $RPC --private-key $PK",
    "removeMinter": "cast send <token> \"removeMinter(address)\" <account> --rpc-url $RPC --private-key $PK",
    "isMinter": "cast call <token> \"isMinter(address)(bool)\" <account> --rpc-url $RPC",
    "addRecoveryOperator": "cast send <token> \"addRecoveryOperator(address)\" <account> --rpc-url $RPC --private-key $PK",
    "removeRecoveryOperator": "cast send <token> \"removeRecoveryOperator(address)\" <account> --rpc-url $RPC --private-key $PK",
    "isRecoveryOperator": "cast call <token> \"isRecoveryOperator(address)(bool)\" <account> --rpc-url $RPC",
...
def drift_report(surface: dict, issuance_path: Path) -> list[str]:
    ...
    for name in sorted(agent_ops):
        if name not in manual and name in {
            ...
            "forcedTransfer", "proposeRecoveryAddress", "executeRecoveryAddress",
            "depositDividend", "dividendOf", "holderCount",
        }:
