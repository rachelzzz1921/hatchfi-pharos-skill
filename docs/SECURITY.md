...
本文件总结 `src/CompliantRWAToken.sol`（同时随包提供 `assets/rwa/CompliantRWAToken.sol`）的安全姿态、威胁模型、一次内部审计的发现，以及每条的处置方式。下述所有修复均有 Foundry 测试覆盖（`forge test`：45 passed；0 failed，含一个 fuzz 不变量）。
