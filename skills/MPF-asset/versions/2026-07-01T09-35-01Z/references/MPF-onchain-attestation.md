...
Python reference (agent may run inline):
...
def evidence_hash(evidence: list) -> str:
    def canon_item(e):
        out = {k: e[k] for k in ("check", "cmd", "flag", "infer", "result") if k in e}
        if "result" in out and isinstance(out["result"], dict):
            out["result"] = dict(sorted(out["result"].items()))
        return out
    payload = json.dumps(
        [canon_item(e) for e in sorted(evidence, key=lambda x: x["check"])],
        separators=(",", ":"),
        ensure_ascii=False,
    )
    return "0x" + hashlib.sha3_256(payload.encode()).hexdigest()  # use keccak in production via cast
```

**Production path** — write canonical JSON to `/tmp/evidence.json`, then:

```bash
cast keccak $(cat /tmp/evidence.json)
```
